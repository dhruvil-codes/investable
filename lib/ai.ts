...
async function callGemini(pitchText: string): Promise<GeminiAnalysis> {
...
async function callGroq(pitchText: string): Promise<GeminiAnalysis> {
...
async function callOpenRouter(pitchText: string): Promise<GeminiAnalysis> {
...
export async function callAI(pitchText: string): Promise<GeminiAnalysis> {
  const providers = [
    { name: "primary", fn: callGemini },
    { name: "fallback1", fn: callGroq },
    { name: "fallback2", fn: callOpenRouter },
  ];
  ...
  for (const { name, fn } of providers) {
    try {
      const result = await fn(pitchText);
      return result;
    } catch (err) {
  ...
  throw new Error(
    `AI analysis unavailable. ${errors.join(" | ")}`
  );
}
